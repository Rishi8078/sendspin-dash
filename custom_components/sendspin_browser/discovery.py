...
class SendspinPingView(HomeAssistantView):
    """API view for browsers to report they are actively connected."""

    url = f"{STATIC_URL_PREFIX}/ping"
    name = f"api:{DOMAIN}:ping"
    requires_auth = True

    async def post(self, request):
        """Handle POST: record player_id and name."""
        hass: HomeAssistant = request.app["hass"]
        try:
            ...
            if player_id:
                players = hass.data.setdefault(DOMAIN, {}).setdefault("players", {})
                players[player_id] = {
                    "player_id": player_id,
...
class SendspinPlayersView(HomeAssistantView):
    """API view to list currently active Sendspin browser players."""
    ...
    async def get(self, request):
        """Return list of active players (pinged within last 60 seconds)."""
        hass: HomeAssistant = request.app["hass"]
        
        players = hass.data.get(DOMAIN, {}).get("players", {})
        now = time.time()
        
        # Filter out players that haven't pinged in 60 seconds
        active_players = [
            p for p in players.values() if now - p["last_seen"] < 60
        ]
        
        return self.json(active_players)

"""mDNS discovery API for Sendspin servers."""

from __future__ import annotations

import asyncio
import logging
from typing import Any

from homeassistant.components import zeroconf
from homeassistant.core import HomeAssistant
from homeassistant.helpers.http import HomeAssistantView
from zeroconf import IPVersion, ServiceStateChange
from zeroconf.asyncio import AsyncServiceBrowser, AsyncServiceInfo

from .const import DOMAIN, SENDSPIN_SERVER_TYPE, STATIC_URL_PREFIX

_LOGGER = logging.getLogger(__name__)

DISCOVERY_TIMEOUT = 3.0
BROWSE_WAIT = 2.0


def _service_info_to_entry(service: AsyncServiceInfo) -> dict[str, Any] | None:
    """Build a {name, url} dict from resolved service info."""
    addresses = service.ip_addresses_by_version(IPVersion.All)
    if not addresses:
        return None

    host = None
    for addr in addresses:
        if not addr.is_link_local and not addr.is_unspecified:
            host = str(addr)
            break
    if host is None:
        host = str(addresses[0])

    port = service.port or 8927

    path = "/sendspin"
    if service.properties:
        raw = service.properties.get(b"path")
        if raw:
            path = raw.decode("utf-8", errors="replace")

    url = f"http://{host}:{port}"
    name = (service.name or "").removesuffix(f".{service.type}").replace(
        "._sendspin-server._tcp.local", ""
    )
    return {"name": name or url, "url": url, "path": path}


class SendspinDiscoveryView(HomeAssistantView):
    """API to discover Sendspin servers on the network via mDNS."""

    url = f"{STATIC_URL_PREFIX}/servers"
    name = f"api:{DOMAIN}:servers"
    requires_auth = True

    async def get(self, request):
        hass: HomeAssistant = request.app["hass"]
        found: list[tuple[str, str]] = []

        def _on_change(zc, service_type: str, name: str, state_change: ServiceStateChange) -> None:
            if state_change == ServiceStateChange.Added:
                found.append((service_type, name))

        try:
            aio_zc = await zeroconf.async_get_async_instance(hass)
            zc = aio_zc.zeroconf
        except Exception as exc:
            _LOGGER.warning("Zeroconf not available: %s", exc)
            return self.json([])

        browser = AsyncServiceBrowser(zc, [SENDSPIN_SERVER_TYPE], handlers=[_on_change])
        try:
            await asyncio.sleep(BROWSE_WAIT)
        finally:
            await browser.async_cancel()

        servers: list[dict[str, Any]] = []
        for stype, sname in found:
            try:
                info = AsyncServiceInfo(stype, sname)
                await info.async_request(zc, DISCOVERY_TIMEOUT)
                entry = _service_info_to_entry(info)
                if entry and not any(s["url"] == entry["url"] for s in servers):
                    servers.append(entry)
            except Exception as exc:
                _LOGGER.debug("Resolve %s failed: %s", sname, exc)

        return self.json(servers)
